...
We study this regime concretely through Starsim, an open-source, agent-based framework for modeling infectious diseases developed and maintained by our team. Starsim is a representative hard case: it has a small training-data footprint, a frequently updated API, and enough methodological subtlety that using it correctly demands genuine domain expertise. Because we maintain it, we are also well placed to judge whether an AI's answers are actually correct. To test how current tools cope, we wrote a quantitative proficiency exam for the framework, built a set of domain-specific AI tools for it (MCP servers and custom "skills"), and measured how a range of LLMs performed as we granted or blocked their access to those tools.
